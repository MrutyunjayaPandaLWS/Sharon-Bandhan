...
public enum Reachability_Error: Error {
...
public let Reachability_Changed_Notification = NSNotification.Name("ReachabilityChangedNotification")

public extension Notification.Name {
    static let reachabilityChanged = Notification.Name("reachabilityChanged")
}

public class ReachabilityAutoSync {

    public typealias NetworkReachable = (ReachabilityAutoSync) -> ()
    public typealias NetworkUnreachable = (ReachabilityAutoSync) -> ()
    ...
    public convenience init(hostname: String,
        ...
        guard let ref = SCNetworkReachabilityCreateWithName(nil, hostname) else {
            throw Reachability_Error.failedToCreateWithHostname(hostname, SCError())
        }
    ...
    public convenience init(queueQoS: DispatchQoS = .default,
        ...
        guard let ref = SCNetworkReachabilityCreateWithAddress(nil, &zeroAddress) else {
            throw Reachability_Error.failedToCreateWithAddress(zeroAddress, SCError())
        }
...
public extension ReachabilityAutoSync {
    ...
    func startNotifier() throws {
        ...
        if !SCNetworkReachabilitySetCallback(reachabilityRef, callback, &context) {
            stopNotifier()
            throw Reachability_Error.unableToSetCallback(SCError())
        }

        if !SCNetworkReachabilitySetDispatchQueue(reachabilityRef, reachabilitySerialQueue) {
            stopNotifier()
            throw Reachability_Error.unableToSetDispatchQueue(SCError())
        }
...
fileprivate extension ReachabilityAutoSync {
    ...
    func setReachabilityFlags() throws {
        try reachabilitySerialQueue.sync { [unowned self] in
            ...
            if !SCNetworkReachabilityGetFlags(self.reachabilityRef, &flags) {
                self.stopNotifier()
                throw Reachability_Error.unableToGetFlags(SCError())
            }
...
extension SCNetworkReachabilityFlags {
    ...
    typealias Connection = ReachabilityAutoSync.Connection
...
private class ReachabilityWeakifier {
    weak var reachability: ReachabilityAutoSync?
    init(reachability: ReachabilityAutoSync) {
